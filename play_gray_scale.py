...
# 顏色.
CONST_RED           = (255, 0, 0)
CONST_GRAY          = (107,130,114)
CONST_GRAY_BLOCK    = (20,31,23)
...
# 玩家飛機.
aircraft_sprite = Sprite(pygame, canvas, 15, 8, [0, 550, 3, 3], CONST_GRAY_BLOCK, CONST_AIRCRAFT_PIXEL) 
# 玩家飛機-飛彈.
aircraft_bullet_sprite = Sprite(pygame, canvas, 1, 4, [0, 0, 2, 2], CONST_GRAY_BLOCK, CONST_AIRCRAFT_BULLET_PIXEL)
...
for i in range(0, 5):
    ...
    aircraft_life_sprite[i] = Sprite(pygame, canvas, 15, 8, [x, y, 2, 2], CONST_GRAY_BLOCK, CONST_AIRCRAFT_PIXEL) 
    x += 35
# 飛碟.
enemy_ufo_sprite = Sprite(pygame, canvas, 16, 7, [-16, 60, 3, 3], CONST_GRAY_BLOCK, CONST_ENEMY_UFO, [60])
enemy_ufo_sprite.visible = False
...
for i in range(0,55):
    ...
    if (i < 11):
        enemy_sprite.append([])
        enemy_sprite[i] = Sprite(pygame, canvas, 8, 8, [0, 0, 3, 3], CONST_GRAY_BLOCK, CONST_ENEMY_PIXEL_1, [30])
    # 敵人2-2,3.
    elif (i < 33):
        enemy_sprite.append([])
        enemy_sprite[i] = Sprite(pygame, canvas, 11, 8, [0, 0, 3, 3], CONST_GRAY_BLOCK, CONST_ENEMY_PIXEL_2, [20])
    # 敵人3-4,5.
    elif (i < 55):
        enemy_sprite.append([])
        enemy_sprite[i] = Sprite(pygame, canvas, 12, 8, [0, 0, 3, 3], CONST_GRAY_BLOCK, CONST_ENEMY_PIXEL_3, [10])
# 敵人子彈
enemy_bullet_sprite = []
for i in range(0,32):
    enemy_bullet_sprite.append([])
    enemy_bullet_sprite[i] = Sprite(pygame, canvas, 3, 7, [0, 0, 3, 3], CONST_GRAY_BLOCK, CONST_ENEMY_BULLET_PIXEL)
...
for i in range(0,4):
    ...
    enemy_boom_sprite[i] = Sprite(pygame, canvas, 11, 9, [0, 0, 3, 3], CONST_GRAY_BLOCK, CONST_ENEMY_BOOM_PIXEL)
...
while running:
    ...
    if(game_mode == 10):
        ...
        for i in range(0,len(enemy_sprite)):
            ...
            if (enemy_sprite[i].visible and aircraft_bullet_sprite.visible and aircraft_bullet_sprite.colliderect(enemy_sprite[i].getCollisionRect()) ):
                ...
                aircraft_bullet_sprite.visible = False                
                # 敵人爆炸像素.
                setBoom( enemy_sprite[i].x, enemy_sprite[i].y, CONST_GRAY_BLOCK)
                # 分數.
        ...
        for i in range(0,len(enemy_bullet_sprite)):
            ...
            if(enemy_bullet_sprite[i].visible):
                ...
                if(aircraft_sprite.visible and enemy_bullet_sprite[i].colliderect(aircraft_sprite.getCollisionRect()) ):
                    ...
                    # 顯示爆炸.
                    setBoom( aircraft_sprite.x, aircraft_sprite.y, CONST_GRAY_BLOCK)
        ...
        if (enemy_ufo_sprite.visible and aircraft_bullet_sprite.colliderect(enemy_ufo_sprite.getCollisionRect()) ):
            ...
            enemy_ufo_sprite.visible = False
            # 飛碟爆炸像素.
            setBoom( enemy_ufo_sprite.x, enemy_ufo_sprite.y, CONST_GRAY_BLOCK)
            # 分數.
    ...
    # 20 : GameOver.
        ...
        # 顯示訊息
        showFont( u"Game Over", CONST_GRAY_BLOCK, 350 , 250)
    # 30 : 下一關.
    elif (game_mode == 30):
        # 初始遊戲.
        initGame()
    ...
    # 分數.
    showFont( u"SCORE", CONST_GRAY_BLOCK, 200, 8)
    showFont( str(player_score), CONST_GRAY_BLOCK, 200, 24)
    # 剩餘數量.
    showFont( u"QUANTITY", CONST_GRAY_BLOCK, 370, 8)
    showFont( str(enemy_quantity), CONST_GRAY_BLOCK, 370, 24)
    # 最高分數.
    showFont( u"HI-SCORE", CONST_GRAY_BLOCK, 540, 8)
    showFont( str(player_hi_score), CONST_GRAY_BLOCK, 540, 24)
    # 顯示FPS.
    if(enable_fps):
